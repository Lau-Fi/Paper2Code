...
class Dastan:
    ...
    def __GetSquareReference(self, Description):
        return Dastan.__GetValidInt("Enter the square " + Description + " (row number followed by column number): ")

    def __UseMoveOptionOffer(self):
        ReplaceChoice = Dastan.__GetValidInt("Choose the move option from your queue to replace (1 to 5): ")
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = Dastan.__GetValidInt("Choose move option to use from queue (1 to 3) or 9 to take the offer: ")
                if Choice == 9:
            ...
            while not SquareIsValid:
                ...
                if not SquareIsValid:
                    print("That Square is not valid haha")
    ...
    @staticmethod
    def __GetValidInt(input_message):
        try:
